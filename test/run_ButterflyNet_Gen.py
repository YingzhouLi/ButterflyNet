...
gen_data_fname = sys.argv[5]

print(data_fname)
print(gen_data_fname)
...
gen_mat = spio.loadmat(gen_data_fname,squeeze_me=False)

gen_n_test = int(gen_mat['n_test'])
gen_x_test = np.float32(gen_mat['x_test'])
gen_x_test = np.reshape(gen_x_test,(gen_n_test,in_siz,1))
gen_y_test = np.float32(gen_mat['y_test'])
gen_y_test = np.reshape(gen_y_test,(gen_n_test,out_siz,1))
