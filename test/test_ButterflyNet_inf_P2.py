...
# freqmag = np.fft.ifftshift(gaussianfun(np.arange(-N//2,N//2),[-4,4],[4,4]))
stepfun = np.zeros(N)
stepfun[N//2-8:N//2-4] = 1/8
stepfun[N//2+4:N//2+8] = 1/8
...
for it in range(Ntest):
    ...
    temp_test_y_prefix = sess.run(y_test_output, feed_dict=test_dict)
    test_y_prefix.append(temp_test_y_prefix)
    print("Iter # %6d: Test Loss: %10e." % (it+1,temp_test_loss))
    sys.stdout.flush()

np.save("./tftmp/hist_loss_test_P2.npy",hist_loss_test_prefix)
np.save("./tftmp/test_y_P2.npy",test_y_prefix)
